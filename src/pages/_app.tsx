...
export default function App({ Component, pageProps }: AppProps) {
  ...
  const scaledAppContent = (
    <div className="app-scale-wrapper">
      <div className="app-scale-content">{appContent}</div>
    </div>
  );

  return (
    <DialogProvider>
      <ActivityLoggerProvider>{isLoginPage ? appContent : scaledAppContent}</ActivityLoggerProvider>
    </DialogProvider>
  );
}
